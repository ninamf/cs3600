  ...
  def elapseTime(self, gameState):
    ...
    for p in self.legalPositions:
        ...
        for newPos in newPosDist.keys():
            ...
            self.beliefs[newPos] += self.beliefs[p] * prob
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState, numParticles=300):
    ...
    self.particlePositions = [random.choice(self.legalPositions) for p in xrange(self.numParticles)] #randomly assigning particles
    
    

                    
    #need to randomly assign particles to places on board
...
#    self.beliefs.normalize()
  ...
  def observe(self, observation, gameState):
    ...
    pacmanBeliefs = util.Counter()
    
    for p in self.particlePositions:
      trueDistance = util.manhattanDistance(p, pacmanPosition)
      pacmanBeliefs[p] += emissionModel[trueDistance]
      
    if pacmanBeliefs.totalCount() == 0.0: #if beiefs is empty then need to initialize the particles uniformly
      if gameState.getLivingGhosts()[self.ghostAgent.index]:
        self.initializeUniformly(gameState)
      else:
        self.particlePositions = []
    else:
      self.particlePositions = [util.sample(pacmanBeliefs) for i in range(self.numParticles)]
  ...
  def elapseTime(self, gameState):
    ...
    #need to updated particle list after ghosts move
    
    updatedParticles = []
    
    for p in self.particlePositions:
      newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p)) #probability of ghost moving to a certain position
      updatedParticles.extend([util.sample(newPosDist)])
      
    self.particlePositions = updatedParticles #update particle list
  ...
  def getBeliefDistribution(self):
    ...
    #for each time a position occurs in the particle list, add 1 to that position in pacman beliefs then normalize - highest prob = highest num particles in that position
    
    pacmanBeliefs = util.Counter()
    
    for p in self.particlePositions:
      pacmanBeliefs[p] += 1 #add 1 to that positions in belief distribution
      
    pacmanBeliefs.normalize()
    
    return pacmanBeliefs
