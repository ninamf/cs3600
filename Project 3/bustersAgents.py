...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for d in livingGhostPositionDistributions:
        ghostProbDict.update(d)
        
    sortedGhostPosDist = dict(sorted(ghostProbDict.iteritems(), key=operator.itemgetter(1), reverse=True)[:4])
    
    ghostPositions = sortedGhostPosDist.keys()
    ...
    for a in legal:
        ...
        succDistance = self.distancer.getDistance(succPos, p)
        if succDistance < mini:
            mini = succDistance
            action = a
